...
const UserProfileDetails = ({ details, streak, onDelete, onEdit }) => {
  return (
    <>
      ...
      <div className='containerdd'>
        ...
        <div className='delete-profile'>
          <span style={{ fontSize: '1rem' }}><MdDelete onClick={onDelete} fontSize={'1.5rem'} color='red' /></span>
        </div>
      </div>
    </>
  );
...
const fetchUserData = async () => {
  ...
  const deleteuser = async () => {
    try {
      ...
      await axios.delete('http://127.0.0.1:8000/api/user/profiles/detail/', {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken}`,
        },
      ...
      setDetails(null);
      toast("Profile Details Deleted Successfully");
    } catch (error) {
  ...
  return (
    <>
     ...
     <div className={`userProfileContainer ${loading ? 'blurred' : ''}`}>
          {loading ? (
           ...
           />
            ...
            details ? (
              ...
              <p className="loadingText">No profile data available.</p>
            )
          )}
        </div>
      </div>
